...
FtpSession.prototype = {
	...
	getFileAndInsertToCache: function (callback, This, extension, file, service, id) {
		var handler = null;
		...
		if (this.validatePathLengthForDirectory(currPath)) {
			...
			if (!this.validateFormatMatchesToFileExtension(fileExtension, extension)) {
				...
				if (itemInCache != null) {
					...
					this.getFileAndInsertToCache(callback, this, extension, file, service, id);
				}
			}
		}
	},

	handleXmlListResponse: function (listResponse, callback, This, service, currentList, map, pager, currentClient, handler, filter, retrieveOnePageOnly) {
		...
		if (error != null) {
			...
			if (!retrieveOnePageOnly && objects.length == pager.pageSize && pager.pageIndex <= maxPageIndex){
				pager.pageIndex++;
	...
	handleICalListResponse: function (listResponse, callback, currentList, This, service, pager, currentClient, handler, filter, retrieveOnePageOnly) {
		var eventsRegex = /BEGIN:VERROR([\s\S]*?)END:VERROR/g;
		...
		if (eventError != null) {
			...
			if (results != null) {
				for (var i = 0; i < results.length; i++) {
					...
					event = 'BEGIN:VCALENDAR' + os.EOL + event + os.EOL + 'END:VCALENDAR';
				...
				if (!retrieveOnePageOnly && results.length == pager.pageSize && pager.pageIndex <= maxPageIndex) {
					pager.pageIndex++;
					currentClient[service].listAction(handler, filter, pager);
				}
				else {
	...
	handleDefaultListResponse: function (listResponse, This, service, currentList, map, pager, currentClient, handler, filter, callback, retrieveOnePageOnly) {
		...
		if (!retrieveOnePageOnly && listResponse.objects.length == pager.pageSize && pager.pageIndex <= maxPageIndex){
			pager.pageIndex++;
			currentClient[service].listAction(handler, filter, pager);
		}
		else {
	...
	handleValidFileDirectory: function (currPath, callback, contents, currentClient) {
		...
		handler = function (listResponse, err) {
			...
			if (listResponse === null) {
				...
				switch (extension) {
					case 'xml':
						This.handleXmlListResponse(listResponse, callback, This, service, currentList, map, pager, currentClient, handler, filter, retrieveOnePageOnly);
						break;
					case 'ical':
						This.handleICalListResponse(listResponse, callback, currentList, This, service, pager, currentClient, handler, filter, retrieveOnePageOnly);
						break;
					default:
						This.handleDefaultListResponse(listResponse, This, service, currentList, map, pager, currentClient, handler, filter, callback, retrieveOnePageOnly);
						break;
				}
			}
		};
				...
				if (fileItem != null) {
					...
					switch (format) {
						case 'xml':
							...
							if (date != null) {
								lastUpdatedDate = new Date(date * 1000);
							}
							fileSize = fileItem.toString().length;
							break;
						case 'ical':
						...
						default:
							...
							fileSize = (JSON.stringify(fileItem)).length;
							if (date != null) {
								lastUpdatedDate = new Date(date * 1000);
							}
							break;
