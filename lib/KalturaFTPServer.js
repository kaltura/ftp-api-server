...
FtpSession.prototype = {
	...
	handleXmlListResponse2: function (listResponse, callback, service, currentList, map, pager, currentClient, handler, filter, retrieveOnePageOnly) {
	...
	handleICalListResponse2: function (listResponse, callback, currentList, service, pager, currentClient, handler, filter, retrieveOnePageOnly) {
		var eventsRegex = /BEGIN:VERROR([\s\S]*?)END:VERROR/g;
		var This = this;
		var eventError = listResponse.match(eventsRegex);
		if (eventError != null) {
	...
	handleDefaultListResponse2: function (listResponse, service, currentList, map, pager, currentClient, handler, filter, callback, retrieveOnePageOnly) {
		var This = this;
		for (var i = 0; i < listResponse.objects.length; i++) {
			This.lists[service].push(listResponse.objects[i]);
			currentList.push(listResponse.objects[i].id + '.' + map.format);
	...
	handleValidFileDirectory2: function (currPath, callback, contents, currentClient) {
		var parsedPath = currPath.split("/");
		...
		handler = function (listResponse, err) {
			...
			if (listResponse === null) {
				...
				switch (extension) {
					case 'xml':
						This.handleXmlListResponse(listResponse, callback, service, currentList, map, pager, currentClient, handler, filter, retrieveOnePageOnly);
						break;
					case 'ical':
						This.handleICalListResponse(listResponse, callback, currentList, service, pager, currentClient, handler, filter, retrieveOnePageOnly);
						break;
					default:
						This.handleDefaultListResponse(listResponse, service, currentList, map, pager, currentClient, handler, filter, callback, retrieveOnePageOnly);
						break;
				}
			}
		};
